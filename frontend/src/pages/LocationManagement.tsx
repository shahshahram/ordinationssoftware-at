...
const LocationManagement: React.FC = () => {
  ...
  const handleLogoUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const targetLocation = selectedLocationForLetterhead || editingLocation;
    ...
    try {
      ...
      if (response.data?.success) {
        ...
        if (selectedLocationForLetterhead) {
          // Aktualisiere selectedLocationForLetterhead
          const updatedLocations = await dispatch(fetchLocations()).unwrap();
          const updatedLocation = Array.isArray(updatedLocations) 
            ? updatedLocations.find((loc: Location) => loc._id === targetLocation._id)
            : null;
          if (updatedLocation) {
            setSelectedLocationForLetterhead(updatedLocation);
  ...
  const handleLogoDelete = async () => {
    ...
    try {
      ...
      if (selectedLocationForLetterhead) {
        // Aktualisiere selectedLocationForLetterhead
        const updatedLocations = await dispatch(fetchLocations()).unwrap();
        const updatedLocation = updatedLocations.find((loc: Location) => loc._id === targetLocation._id);
        if (updatedLocation) {
          setSelectedLocationForLetterhead(updatedLocation);
        }
      } else {
                  ...
                  if (location) {
                    ...
                    if (location.logo) {
                      ...
                      if (location.logo.path) {
                        // Wenn path vorhanden, verwende diesen (kann relativ oder absolut sein)
                        ...
                        if (sourceLocationId) {
                          try {
                            const response = await api.post(`/locations/${selectedLocationForTemplates._id}/letter-templates/import`, {
                              sourceLocationId,
                              templateIndices: [] // Alle importieren - später Dialog für Auswahl
                            });
          ...
          <Stack spacing={2} sx={{ mt: 1 }}>
            ...
            <Box>
              ...
              <RichTextEditor
                ...
                placeholder="Beginnen Sie mit der Eingabe... Verwenden Sie Platzhalter wie [Name], [Patient], [Datum], [Standort]"
                minHeight={200}
              />
            </Box>
            <Alert severity="info">
              <Typography variant="body2">
                ...
                [Name] - Arztname<br />
                [Patient] - Patientenvollname<br />
                [Datum] - Aktuelles Datum<br />
                [Standort] - Standortname<br />
                [ArztTitel] - Arzt-Titel
              </Typography>
            </Alert>
            <FormControlLabel
              control={
